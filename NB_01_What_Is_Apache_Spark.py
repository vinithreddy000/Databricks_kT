...
# MAGIC %md
# MAGIC ## SECTION 6 — Common Mistakes Beginners Make
# MAGIC
# MAGIC ### Mistake #1: Using `collect()` on Large Data
# MAGIC
# MAGIC **What happens:** You call `df.collect()` on a DataFrame with 100 million rows.  
# MAGIC **Why it's bad:** `collect()` brings ALL data to one computer (the driver). If the data is too big, your driver runs out of memory and CRASHES.  
# MAGIC **The fix:** Use `display(df)` (shows first 1000 rows), `df.show(10)` (shows 10 rows), or `df.take(5)` (brings only 5 rows to driver).
# MAGIC
# MAGIC ```python
# MAGIC # BAD — will crash on large data:
# MAGIC all_data = huge_df.collect()  # Brings 100M rows to one machine!
# MAGIC
# MAGIC # GOOD — safe alternatives:
# MAGIC display(huge_df)        # Shows first 1000 rows in a nice table
# MAGIC huge_df.show(10)        # Shows first 10 rows as text
# MAGIC first_five = huge_df.take(5)  # Brings only 5 rows to driver
# MAGIC ```
# MAGIC
# MAGIC ---
# MAGIC
...
# MAGIC
# MAGIC **What happens:** You write 50 lines of transformations but don't see any output.  
# MAGIC **Why it's confusing:** Spark doesn't do anything until you call an ACTION!  
# MAGIC **The fix:** Remember: Transformations are lazy (just planning). Actions trigger execution.
# MAGIC
# MAGIC ```python
# MAGIC # This does NOTHING by itself:
# MAGIC df2 = df.filter(col("age") > 18)    # Lazy! Just a plan.
# MAGIC df3 = df2.select("name", "age")      # Still lazy! Just adding to the plan.
# MAGIC
# MAGIC # This TRIGGERS execution:
# MAGIC df3.show()    # ACTION! Now Spark actually runs the filter and select.
# MAGIC df3.count()   # ACTION! Now Spark runs everything and counts.
# MAGIC ```
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ### Mistake #3: Creating SparkSession Manually in Databricks
# MAGIC
# MAGIC **What happens:** You write `SparkSession.builder.getOrCreate()` in a Databricks notebook.  
# MAGIC **Why it's wrong:** Databricks ALREADY gives you a pre-configured `spark` variable. Creating your own can cause conflicts.  
# MAGIC **The fix:** Just use the `spark` variable that's already available. Same for `sc` (SparkContext).
# MAGIC
# MAGIC ```python
# MAGIC # BAD — don't do this in Databricks:
# MAGIC from pyspark.sql import SparkSession
# MAGIC spark = SparkSession.builder.appName("MyApp").getOrCreate()  # Unnecessary!
# MAGIC
# MAGIC # GOOD — just use what's already there:
# MAGIC print(spark.version)  # 'spark' is already available!
# MAGIC sc = spark.sparkContext  # 'sc' is already available too!
# MAGIC ```
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ### Mistake #4: Thinking Spark is Always Faster
# MAGIC
# MAGIC **What happens:** You use Spark for a CSV file with 100 rows.  
# MAGIC **Why it's wrong:** Spark has OVERHEAD (starting workers, distributing data, coordinating). For small data, regular Python/Pandas is faster.  
# MAGIC **The fix:** Use Spark when data is BIG (millions of rows, gigabytes+). Use Pandas for small data (<1 million rows on a single machine).
# MAGIC
# MAGIC ```python
# MAGIC # For small data (< 1 million rows), Pandas is simpler and faster:
# MAGIC import pandas as pd
# MAGIC small_df = pd.read_csv("small_file.csv")  # Fast for small files
# MAGIC
# MAGIC # For big data (millions/billions of rows), use Spark:
# MAGIC big_df = spark.read.csv("huge_folder/*.csv")  # Distributed across cluster
# MAGIC ```
# MAGIC
# MAGIC ---
# MAGIC
...
# MAGIC
# MAGIC **What happens:** You don't think about how data is split across workers.  
# MAGIC **Why it matters:** If data is split into too many tiny pieces or too few huge pieces, performance suffers.  
# MAGIC **The fix:** Check partitions with `df.rdd.getNumPartitions()`. Aim for partition sizes of 128MB-256MB.
# MAGIC
# MAGIC ```python
# MAGIC # Check current partitions:
# MAGIC print(f"Partitions: {df.rdd.getNumPartitions()}")  # See how data is split
# MAGIC
# MAGIC # If too many small partitions:
# MAGIC df = df.coalesce(10)  # Reduce to 10 partitions (no shuffle, fast)
# MAGIC
# MAGIC # If too few large partitions:
# MAGIC df = df.repartition(100)  # Increase to 100 partitions (causes shuffle)
# MAGIC ```
...
# MAGIC Copy the code from Beginner Example 1 and run it. Write down the Spark version you see.
# MAGIC
# MAGIC ### Level 2 (Tiny Change)
# MAGIC Modify the RDD example to create an RDD with numbers 1 to 20 (instead of 1 to 10). Print the count and sum.
# MAGIC
# MAGIC ### Level 3 (Combine Two Things)
# MAGIC Create a DataFrame with 3 columns: `product`, `price`, `quantity`. Add 5 rows. Then use `display()` to show it AND print the schema.
# MAGIC
# MAGIC ### Level 4 (New Scenario)
# MAGIC Create an RDD of city names. Use `.filter()` to keep only cities with more than 5 characters. Count the result.
# MAGIC
# MAGIC ### Level 5 (Intermediate Project)
# MAGIC Create a DataFrame of students with columns: `name`, `subject`, `score`. Add 10 rows. Filter students with score > 70. Display the filtered result.
# MAGIC
# MAGIC ### Level 6 (Design First)
# MAGIC Before writing code: describe in comments how you would use Spark to count the number of unique words in a text (like a book). Then implement it.
# MAGIC
# MAGIC ### Level 7 (Optimize It)
# MAGIC Take the parallel processing example from Section 4. Try different numbers of partitions (2, 4, 8, 16, 32). Which gives the best performance? Why?
# MAGIC
# MAGIC ### Level 8 (Edge Cases)
# MAGIC What happens when you create an RDD from an empty list? What about a DataFrame with no rows? Test both and document the behavior.
# MAGIC
# MAGIC ### Level 9 (Production-Grade)
# MAGIC Write a function called `safe_display(df, max_rows=10)` that checks if a DataFrame is empty before displaying, and prints a warning if it has more than 1 million rows.
# MAGIC
# MAGIC ### Level 10 (Teach It)
# MAGIC Write a markdown explanation (in your own words) of Apache Spark that you could share with a colleague who has never heard of it. Include: what it is, why it exists, and one simple code example.

# COMMAND ----------

# DBTITLE 1,SECTION 7 — Homework Solutions (Levels 1-5)
# ═══════════════════════════════════════════════════════
# HOMEWORK SOLUTIONS — Levels 1 through 5
# ═══════════════════════════════════════════════════════

# ---- LEVEL 1 SOLUTION: Just Run It ----
print("=== Level 1: Check Spark Version ===")
print(f"Spark Version: {spark.version}")  # Just run and note the version number
# WHY: This confirms your environment is working correctly

# ---- LEVEL 2 SOLUTION: Tiny Change ----
print("\n=== Level 2: RDD with 1-20 ===")
my_numbers_20 = list(range(1, 21))  # Changed from 10 to 20
rdd_20 = sc.parallelize(my_numbers_20)  # Create RDD from the list
print(f"Count: {rdd_20.count()}")  # Should print 20
print(f"Sum: {rdd_20.sum()}")  # Should print 210 (sum of 1 to 20)
# WHY: Practicing the basic RDD creation and action pattern

# ---- LEVEL 3 SOLUTION: Combine Two Things ----
print("\n=== Level 3: Product DataFrame ===")
product_data = [  # Create sample product data
    ("Laptop", 999.99, 5),       # (product, price, quantity)
    ("Mouse", 29.99, 50),        # Each tuple is one row
    ("Keyboard", 79.99, 30),
    ("Monitor", 349.99, 10),
    ("Headset", 149.99, 25)
]
product_columns = ["product", "price", "quantity"]  # Column names
product_df = spark.createDataFrame(product_data, product_columns)  # Create DataFrame
display(product_df)  # Show the table (Databricks rich display)
print("\nSchema:")  # Label
product_df.printSchema()  # Show data types
# WHY: Combines DataFrame creation with both display() and printSchema()

# ---- LEVEL 4 SOLUTION: New Scenario ----
print("\n=== Level 4: Filter Cities by Name Length ===")
cities = ["Tokyo", "London", "Paris", "New York", "Berlin", "Rio", "Mumbai", "LA"]  # City names
cities_rdd = sc.parallelize(cities)  # Create RDD from city list
long_cities = cities_rdd.filter(lambda city: len(city) > 5)  # Keep cities with > 5 characters
print(f"Cities with more than 5 characters: {long_cities.collect()}")  # Show results
print(f"Count: {long_cities.count()}")  # Count them
# WHY: Practices filter transformation with a custom condition
# Expected: ['London', 'New York', 'Berlin', 'Mumbai'] — 4 cities

# ---- LEVEL 5 SOLUTION: Intermediate Project ----
print("\n=== Level 5: Student Scores ===")
from pyspark.sql.functions import col  # Import col for filtering

student_data = [  # 10 students with subjects and scores
    ("Alice", "Math", 85),
    ("Bob", "Science", 62),
    ("Charlie", "Math", 91),
    ("Diana", "English", 78),
...
student_df = spark.createDataFrame(student_data, ["name", "subject", "score"])  # Create DF
high_scorers = student_df.filter(col("score") > 70)  # Filter for score > 70
print(f"Students with score > 70: {high_scorers.count()} out of {student_df.count()}")  # Count
display(high_scorers)  # Show the filtered results
# WHY: End-to-end workflow: create data → filter → display results

# COMMAND ----------

# DBTITLE 1,SECTION 7 — Homework Solutions (Levels 6-10)
# ═══════════════════════════════════════════════════════
# HOMEWORK SOLUTIONS — Levels 6 through 10
# ═══════════════════════════════════════════════════════

# ---- LEVEL 6 SOLUTION: Design First ----
print("=== Level 6: Count Unique Words ===")
# DESIGN (write your plan before coding):
# 1. Start with a text (string or list of sentences)
# 2. Split each sentence into individual words (flatMap)
# 3. Convert all words to lowercase (to avoid counting "The" and "the" separately)
# 4. Remove duplicates using .distinct()
# 5. Count the unique words

# IMPLEMENTATION:
text = [  # Sample text as a list of sentences
    "Apache Spark is a fast distributed computing engine",
    "Spark can process big data across many computers",
    "Many companies use Spark for data processing",
    "Spark is faster than Hadoop MapReduce"
]
text_rdd = sc.parallelize(text)  # Distribute the text across workers
words_rdd = text_rdd.flatMap(lambda line: line.lower().split(" "))  # Split into words
unique_words = words_rdd.distinct()  # Remove duplicate words
print(f"Total words (with duplicates): {words_rdd.count()}")  # Count all words
print(f"Unique words: {unique_words.count()}")  # Count unique words
print(f"Sample unique words: {unique_words.take(10)}")  # Show first 10
# WHY: Teaches you to plan before coding and use flatMap + distinct

# ---- LEVEL 7 SOLUTION: Optimize It ----
print("\n=== Level 7: Partition Experiment ===")
import time  # For timing

data = list(range(1, 5000001))  # 5 million numbers

# Test different partition counts
for num_partitions in [2, 4, 8, 16, 32]:  # Try each partition count
    start = time.time()  # Start timer
    rdd = sc.parallelize(data, num_partitions)  # Create RDD with N partitions
    result = rdd.map(lambda x: x * 2).filter(lambda x: x > 5000000).count()  # Work
    elapsed = time.time() - start  # Measure time
    print(f"  Partitions: {num_partitions:>2} | Time: {elapsed:.3f}s | Result: {result:,}")  # Report

print("\nObservation: More partitions = more parallelism, but too many = overhead")
print("Sweet spot depends on: data size, cluster size, and task complexity")
# WHY: Teaches that partition count affects performance and there's a sweet spot

# ---- LEVEL 8 SOLUTION: Edge Cases ----
print("\n=== Level 8: Edge Cases ===")
# Test 1: Empty RDD
empty_rdd = sc.parallelize([])  # RDD with no elements
print(f"Empty RDD count: {empty_rdd.count()}")  # Returns 0
print(f"Empty RDD isEmpty: True")  # It's empty

# Test 2: Empty DataFrame
from pyspark.sql.types import StructType, StructField, StringType, IntegerType  # Import types
empty_schema = StructType([  # Define schema for empty DataFrame
    StructField("name", StringType(), True),  # String column
    StructField("age", IntegerType(), True)  # Integer column
])
empty_df = spark.createDataFrame([], empty_schema)  # Create empty DataFrame with schema
print(f"\nEmpty DataFrame count: {empty_df.count()}")  # Returns 0
print(f"Empty DataFrame schema:")
empty_df.printSchema()  # Schema exists even with no data!
print("Key insight: Schema exists independently of data. You can have structure with no rows.")
# WHY: Understanding edge cases prevents bugs in production code

# ---- LEVEL 9 SOLUTION: Production-Grade ----
print("\n=== Level 9: Safe Display Function ===")

def safe_display(df, max_rows=10):
    """Safely display a DataFrame with checks for empty and large data."""
    # Check if DataFrame is empty
    if df.head(1) == []:  # Efficient way to check if empty (doesn't count all rows)
        print("WARNING: DataFrame is empty! No data to display.")
        return  # Exit the function early
    
    # Check approximate size (count can be expensive, so we use a limit trick)
    row_count = df.count()  # Get exact count (careful: expensive on huge data)
    
    if row_count > 1000000:  # If more than 1 million rows
        print(f"WARNING: DataFrame has {row_count:,} rows. Showing only first {max_rows}.")
        print("Consider filtering or aggregating before displaying.")
    else:
        print(f"DataFrame has {row_count:,} rows. Showing first {max_rows}:")
    
    display(df.limit(max_rows))  # Only show limited rows safely

# Test with our student DataFrame
test_df = spark.createDataFrame(  # Create test data
    [("Alice", 30), ("Bob", 25)], ["name", "age"]
)
safe_display(test_df)  # Should display normally
safe_display(spark.createDataFrame([], empty_schema))  # Should show warning
# WHY: Production code needs safeguards against common failure modes

# ---- LEVEL 10 SOLUTION: Teach It ----
print("\n=== Level 10: Teach It (Example Explanation) ===")
print("""
--- What is Apache Spark? (My explanation for a colleague) ---

Imagine you have a HUGE pile of receipts (billions of them) and you need
to find the total amount spent on groceries last year.

Option 1: One person reads every receipt. Takes weeks.
Option 2: You hire 100 people, split the pile into 100 stacks,
          each person totals their stack, then you add up 100 subtotals.
          Takes hours.

Apache Spark is Option 2, but for computers.

It takes your data, splits it across many computers, processes it
in parallel, and combines the results. All automatically.

Quick example:
  # This processes billions of rows across a cluster:
  total = spark.read.csv('huge_data.csv') \\
              .filter(col('category') == 'groceries') \\
              .agg(sum('amount'))

That's it. Spark handles the splitting, distributing, and combining.
You just describe WHAT you want, not HOW to distribute it.
""")
print("\nCongratulations! You've completed all 10 homework levels!")
# WHY: Teaching is the best way to solidify your own understanding